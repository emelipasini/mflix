...
export default class MoviesDAO {
    ...
    */
		...
		try {
			...
			const searchedMovies = await cursor.toArray()
	 ...
	 * @param {string[]} casts - The cast members to match with.
	 * @returns {QueryParams} The QueryParams for cast search
	 */
	static castSearchQuery(casts) {
		const searchCast = Array.isArray(casts) ? casts : casts.split(", ")
    ...
    */
		...
		try {
			...
			const count = await (await movies.aggregate(
				countingPipeline,
			)).next()
			return {
				...results,
    ...
    */
		...
		const displayCursor =
			page === 0
				? cursor.limit(moviesPerPage)
				: cursor.limit(moviesPerPage).skip(page * moviesPerPage)
